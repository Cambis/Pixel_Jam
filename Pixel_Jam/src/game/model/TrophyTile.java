...
public class TrophyTile implements Tile {
	...
	public Graphics2D draw(int x, int y, int size, Graphics2D g) {
		...
		if(enabled) {
			...
			g.fillOval((int) (x*size + size/2 - widthScale*size/2), y*size, (int) (size * widthScale), size);

			g.setColor(Color.YELLOW.darker());
			g.drawOval((int) (x*size + size/2 - widthScale*size/2), y*size, (int) (size * widthScale), size);
			g.drawOval((int) (x*size + size/2 - widthScale*size/4), y*size + size/4, (int) (size/2 * widthScale), size/2);
		}
